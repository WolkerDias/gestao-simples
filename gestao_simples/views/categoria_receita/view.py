# views/categoria_receita/view.py
...
from views.categoria_receita.edit import show_edit_categoria_receita

@st.dialog("Categorias de Receita - Detalhes", width="large")
def show_view_categoria_receita(categoria: CategoriaReceita):
    """Exibe os detalhes de uma categoria de receita específica"""
    if not categoria:
    ...
    with col2:
        st.metric(
            ...
            value=len(categoria.receitas) if hasattr(categoria, 'receitas') else 0
        )
    ...
    with col_edit:
        if st.button("✏️ Editar", use_container_width=True):
            show_edit_categoria_receita(categoria)
    
    with col_delete:
        if st.button("🗑️ Excluir", use_container_width=True, type="secondary"):
            show_delete_confirmation_categoria_receita(categoria.id)
    
    with col_back:
        if st.button("⬅️ Voltar", use_container_width=True):
            st.rerun()

def show_delete_confirmation_categoria_receita(categoria_id: int):
    """Exibe confirmação de exclusão de categoria de receita"""
    categoria_service = CategoriaReceitaService()
    
    st.warning("⚠️ Confirmação de Exclusão")
    st.write("Tem certeza que deseja excluir esta categoria? Esta ação não pode ser desfeita e removerá a categoria de todas as receitas associadas.")
